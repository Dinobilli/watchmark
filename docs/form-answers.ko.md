# Codex for Open Source 신청서 초안

Repository URL: https://github.com/Dinobilli/watchmark

## 담당 역할

주 책임자

## 이 리포지터리가 적합하다고 생각하는 이유

Watchmark는 한국 공공기관 공지, 지원사업, 쇼핑몰 상품 페이지, 경쟁사 가격/채용/약관 변화를 추적하는 MIT 오픈소스 모니터입니다. 단순 변경 알림이 아니라 의미 있는 텍스트 변화와 요약을 제공해 반복 확인 업무를 줄입니다. CI, 테스트, 릴리스, 이슈 템플릿, 보안정책, 한글 README와 로드맵을 갖춘 공개 MVP입니다.

## 관심 항목

- Codex Security
- 프로젝트에 사용할 API 크레딧

## 프로젝트에서 API 크레딧을 어떻게 사용할 계획인가요?

API 크레딧은 선택형 AI 요약과 평가 데이터 구축에 사용하겠습니다. 한국어 공공 공지, 쇼핑 상품 페이지, 약관/채용 페이지의 긴 변경 내역을 요약하고, PR마다 fixture 기반 회귀 테스트와 릴리스 노트/이슈 분류 자동화를 돌려 유지관리 부담을 줄이겠습니다. 기본 MVP는 로컬 요약으로 동작해 API 키 없이도 사용할 수 있습니다.

## 더 알려주고 싶은 내용

초기 프로젝트라 지표는 작지만, 문제는 반복적이고 대중적입니다. Watchmark는 한국 사이트 특화 템플릿, SSRF/XSS 방어, noisy 페이지 필터링, self-hosted 우선 구조로 차별화합니다. Codex는 기능 구현뿐 아니라 PR 검토, 보안 점검, 릴리스 워크플로, 기여자 온보딩에 직접 쓰겠습니다.
